...
exports.default = exports.test = exports.ReuseBrowserContextStorage = exports._baseTest = void 0;
...
var _gridClient = require("playwright-core/src/grid/gridClient");
...
class ReuseBrowserContextStorage {
  ...
  async obtainContext(browser, newContextOptions) {
    if (!this._browserContext) return await this._createNewContext(browser);
    return await this._refurbishExistingContext(newContextOptions);
  }

  async _createNewContext(browser) {
    this._browserContext = await browser.newContext();
    this._options = this._browserContext._options;

    this._browserContext.on('page', page => {
      page.on('framenavigated', frame => {
        if (this._pauseNavigationEventCollection) return;
        const origin = new URL(frame.url()).origin;
        if (origin !== 'null') // 'chrome-error://chromewebdata/'
          this._uniqueOrigins.add(origin);
      });
      page.on('crash', () => {
        var _this$_browserContext;

        (_this$_browserContext = this._browserContext) === null || _this$_browserContext === void 0 ? void 0 : _this$_browserContext.close().then(() => {});
  ...
  async _refurbishExistingContext(newContextOptions) {
    (0, _utils.assert)(this._browserContext);
  ...
  async _applyNewContextOptions(page, newOptions) {
    (0, _utils.assert)(this._options);
    const currentViewport = page.viewportSize();
    const newViewport = newOptions.viewport === undefined ? {
      width: 1280,
      height: 720
    } : newOptions.viewport;
    if (((currentViewport === null || currentViewport === void 0 ? void 0 : currentViewport.width) !== (newViewport === null || newViewport === void 0 ? void 0 : newViewport.width) || (currentViewport === null || currentViewport === void 0 ? void 0 : currentViewport.height) !== (newViewport === null || newViewport === void 0 ? void 0 : newViewport.height)) && newViewport !== null && newViewport !== void 0 && newViewport.height && newViewport !== null && newViewport !== void 0 && newViewport.width) await page.setViewportSize(newViewport);
    this._options = newOptions;
  }

  async obtainPage() {
    (0, _utils.assert)(this._browserContext);
    if (this._browserContext.pages().length === 0) return await this._browserContext.newPage();
    return this._browserContext.pages()[0];
  }

}

exports.ReuseBrowserContextStorage = ReuseBrowserContextStorage;
...
const test = _baseTest.extend({
  ...
  _reuseBrowserContext: [new ReuseBrowserContextStorage(), {
    scope: 'worker'
  }],
  context: async ({
    browser,
