...
var _multimap = require("playwright-core/src/utils/multimap");
...
class RawReporter {
  ...
  _serializeSuite(suite) {
    const fileId = (0, _utils.calculateSha1)(suite.location.file.split(_path.default.sep).join('/'));
  ...
  _serializeResult(test, result) {
    return {
      ...
      error: (0, _base.formatResultFailure)(test, result, '', true).tokens.join('').trim(),
